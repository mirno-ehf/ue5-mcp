...
bool FBlueprintMCPServer::ProcessOneRequest()
{
	...
	else if (Req->Endpoint == TEXT("addInterface"))
	{
		Response = HandleAddInterface(Req->Body);
	}
	else if (Req->Endpoint == TEXT("removeInterface"))
	{
		Response = HandleRemoveInterface(Req->Body);
	}
